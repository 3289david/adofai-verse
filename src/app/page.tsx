...
async function getData() {
  try {
    const [featured, mapCount, playerCount, recordCount] = await Promise.all([
      db.map.findMany({
    ...
    return { featured, stats: { maps: mapCount, players: playerCount, records: recordCount } };
  } catch {
    return { featured: [], stats: { maps: 0, players: 0, records: 0 } };
  }
}

function toMapData(m: Prisma.MapGetPayload<{ include: { creator: { select: { id: true; username: true; avatar: true } } } }>): MapData {
  return {
    ...m,
      ...
      <section className="border-b border-line">
        <div className="mx-auto max-w-2xl px-4 py-24 text-center">
          ...
          <p className="text-lg text-soft mb-8">
            Maps, rankings, AI coaching, and deep analytics for<br className="hidden sm:block" /> A Dance of Fire and Ice.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <Link href="/maps"     className="px-7 py-2.5 fire-btn text-sm">Browse Maps <ArrowRight size={13} className="inline ml-1" /></Link>
            <Link href="/register" className="px-7 py-2.5 text-sm font-bold bg-card border border-line rounded-[10px] text-soft hover:text-white hover:border-line-hi transition-colors">Sign Up Free</Link>
          </div>
        </div>
      </section>
      ...
      {featured.length > 0 ? (
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-black text-white">Featured Maps</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {featured.map(m => <MapCard key={m.id} map={toMapData(m)} />)}
          </div>
        </section>
      ) : (
        <section className="mx-auto max-w-2xl px-4 py-24 text-center">
          ...
          <p className="text-soft mb-6">Sign up and be the first to add maps to ADOFAI.VERSE.</p>
          <div className="flex justify-center gap-3">
            <Link href="/register" className="px-6 py-2.5 fire-btn text-sm">Get Started</Link>
