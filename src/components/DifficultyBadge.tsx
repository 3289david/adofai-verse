...
export function DifficultyBadge({ difficulty, size = "md", showLabel = false, className = "" }: DifficultyBadgeProps) {
  const color = getDifficultyColor(difficulty);
  const pad = size === "sm" ? "px-1.5 py-0.5 text-xs" : size === "lg" ? "px-3 py-1.5 text-base" : "px-2 py-1 text-sm";

  return (
    <span
      className={`inline-flex items-center gap-1 rounded font-bold tabular-nums ${pad} ${className}`}
